...
export async function generateMetadata({ params }: Props) {
  ...
  // Load translated name/description (base product fields are in Spanish)
  if (isValidLocale(locale)) {
    const { data: tr } = await supabase
      ...
      .single()
    if (tr) {
      const name = tr.name ?? data.name
      const desc = (tr.description ?? data.description)?.replace(/<[^>]*>/g, '').slice(0, 160)
...
export default async function ProductPage({ params }: Props) {
  ...
  // Load translation for current locale (base product fields are in Spanish)
  ...
  const { data: tr } = await supabase
    ...
    .single()
  if (tr) {
    translatedName = tr.name ?? null
    translatedDescription = tr.description ?? null
  }
        ...
        <div className="max-w-6xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center gap-2 text-xs tracking-widest uppercase text-gray-400 mb-8 flex-wrap">
            ...
            {breadcrumbSegments.map((seg) => (
              <span key={seg.label} className="flex items-center gap-2">
                <span>/</span>
                {seg.slug ? (
                  <Link href={`${base}/products?category=${encodeURIComponent(seg.slug)}`} className="hover:text-[#d4006e] transition-colors">
                    {seg.label}
                  </Link>
                ) : (
                  <span>{seg.label}</span>
          ...
          <div className="grid md:grid-cols-2 gap-16">
            ...
            <div className="flex flex-col gap-8">
              <ProductImageGallery images={images} name={displayName} />

              {/* Specifications — under gallery */}
              {(product.weight_grams || product.width_mm || product.height_mm || product.depth_mm) && (
                <div>
                  <p className="text-xs tracking-widest uppercase text-gray-400 mb-3">{t.product.specifications}</p>
                  <dl className="grid grid-cols-2 gap-x-6 gap-y-2">
                    {product.weight_grams && (
                      <>
                        <dt className="text-xs text-gray-400">{t.product.weight}</dt>
                    ...
                    {(product.width_mm || product.height_mm || product.depth_mm) && (
                      <>
                        <dt className="text-xs text-gray-400">{t.product.packageSize}</dt>
                        <dd className="text-xs text-gray-700">
                          {[product.width_mm, product.height_mm, product.depth_mm]
                            .filter(Boolean)
                            .map((v) => `${v} mm`)
            ...
            <div className="space-y-6 pt-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs tracking-wide text-gray-500">
                  {product.stock_quantity > 0 ? `${t.product.inStock} (${product.stock_quantity})` : t.product.outOfStock}
                </span>
              </div>
